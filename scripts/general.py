...
class GeneralFunctions:
    ...
    async def mute_member(self, message):
        async def launch_on_mute_ended():
            await self.on_mute_ended(message, choice(phrases["on_mute_end"]))

        mute_duration = timedelta(seconds=5 * (self.bad_counter[message.author] - 2))
        ...
        if self.timers[message.author] and not self.timers[message.author].get("mute"):
            self.timers[message.author]["mute"] = Timer(mute_duration, launch_on_mute_ended)
    ...
    async def decrease_reputation(self, message):
        ...
        self.timers[message.author]["reputation"] = Timer(timedelta(hours=(self.bad_counter[message.author] - 2)),
